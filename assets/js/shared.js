...
const db = {
    ...
    // Default bundled blogs for instant 0ms first-paint render
    _DEFAULT_STAY_BLOGS: [
        {
...
<p class="mt-8 font-semibold text-slate-900">Experience true alpine tranquility. <a href="/booking" class="text-[#C5A059] underline hover:text-[#996515]">Book your private luxury chalet in Nathia Gali</a> today with KPH Stay.</p>`,
    ...
    // Synchronous instant cache reader for 0ms visual paint
    getCachedBlogs: () => {
        if (window.KaghanDB_Cache.blogs && window.KaghanDB_Cache.blogs.length > 0) {
            return window.KaghanDB_Cache.blogs;
        }
        try {
            const raw = localStorage.getItem('kph_stay_blogs_cache');
            if (raw) {
                const parsed = JSON.parse(raw);
                if (Array.isArray(parsed) && parsed.length > 0) {
                    window.KaghanDB_Cache.blogs = parsed;
                    return parsed;
                }
            }
        } catch (e) {}
        return KaghanDB._DEFAULT_STAY_BLOGS;
    },

    // Blog Posts CRUD Helpers with SWR Persistent Caching
    getBlogs: async (forceRefresh = false) => {
        // Return memory cache immediately if available and fresh
        if (!forceRefresh && window.KaghanDB_Cache.blogs && window.KaghanDB_Cache.blogs.length > 0) {
            return window.KaghanDB_Cache.blogs;
        }

        // Try local storage cache for instant return while revalidating
        let cached = null;
        try {
            const raw = localStorage.getItem('kph_stay_blogs_cache');
        ...
        if (cached && Array.isArray(cached) && cached.length > 0 && !forceRefresh) {
            window.KaghanDB_Cache.blogs = cached;
            // Silent background revalidation
            setTimeout(() => KaghanDB._revalidateBlogs(), 100);
            return cached;
        }

        return await KaghanDB._revalidateBlogs();
    },

    _revalidateBlogs: async () => {
        try {
            ...
            const finalBlogs = list.length > 0 ? list : KaghanDB._DEFAULT_STAY_BLOGS;
            const sorted = finalBlogs.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
            window.KaghanDB_Cache.blogs = sorted;
            try {
                localStorage.setItem('kph_stay_blogs_cache', JSON.stringify(sorted));
                localStorage.setItem('kph_stay_blogs_cache_time', Date.now().toString());
            } catch (e) {}
            window.dispatchEvent(new CustomEvent('kaghan-db-blogs', { detail: sorted }));
            return sorted;
        } catch (error) {
            console.warn("Firestore blogs fetch fallback:", error);
            const fallback = KaghanDB.getCachedBlogs();
            window.KaghanDB_Cache.blogs = fallback;
            return fallback;
        }
    },
    ...
    getBlogByIdOrSlug: async (idOrSlug) => {
        ...
        return blogs.find(b => 
            ...
            (b.title && b.title.toLowerCase().trim().replace(/\s+/g, '-').replace(/[^\w\-]+/g, '') === target)
        ) || null;
    },

    addBlog: async (blog) => {
        const id = await callAdminAction('addBlog', { blog });
        localStorage.removeItem('kph_stay_blogs_cache');
        window.KaghanDB_Cache.blogs = null;
    ...
    deleteBlog: async (id) => {
        const res = await callAdminAction('deleteBlog', { id });
        localStorage.removeItem('kph_stay_blogs_cache');
        window.KaghanDB_Cache.blogs = null;
        return res;
    },
...
function injectWhatsApp() {
    ...
    chatBox.innerHTML = `
        ...
        <div class="flex-grow p-5 bg-[#E5DDD5] flex flex-col justify-between text-left text-xs">
            ...
            <a href="https://wa.me/923340091127?text=Hi%2C%20I'm%20interested%20in%20booking%20a%20stay%20at%20Kaghan%20Stay." target="_blank" rel="noopener noreferrer" id="wa-chat-btn" class="w-full bg-[#25D366] hover:bg-[#20ba59] text-white font-bold py-2.5 rounded-xl text-center transition-all text-xs flex items-center justify-center gap-2 shadow-md">
...
// === Automatic Header & Footer Blog Nav Link Injector ===
function ensureBlogNavLinks() {
    try {
        ...
        if (navbar) {
            ...
            if (navContainer) {
                ...
                const blogLinkItem = links.find(a => {
                ...
                if (blogLinkItem) {
                    if (blogLinkItem.innerText.trim().toLowerCase() === 'journal') {
                        blogLinkItem.innerText = 'Blog';
                    }
                    if (blogLinkItem.getAttribute('href') === 'blog.html' || blogLinkItem.getAttribute('href') === '/blog.html') {
                        blogLinkItem.setAttribute('href', '/blog');
                    }
                } else {
        ...
        if (drawer) {
            ...
            drawerLinks.forEach(a => {
                const href = (a.getAttribute('href') || '').toLowerCase();
                const text = (a.innerText || '').trim().toLowerCase();
                if (href.includes('blog') || text === 'journal') {
                    a.innerText = 'Blog';
        ...
        footers.forEach(footer => {
            ...
            links.forEach(a => {
                const href = (a.getAttribute('href') || '').toLowerCase();
                const text = (a.innerText || '').trim().toLowerCase();
                if (text.includes('journal') || href.includes('blog')) {
                    if (text === 'resort journal' || text === 'travel journal' || text === 'journal') {
                        a.innerText = 'Travel Blog';
                    }
                    if (href === 'blog.html' || href === '/blog.html') {
                        a.setAttribute('href', '/blog');
                    }
                }
            });
        ...
        console.warn("Error ensuring blog nav links:", e);
    }
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', ensureBlogNavLinks);
} else {
    ensureBlogNavLinks();
}
window.addEventListener('load', ensureBlogNavLinks);

// === Speculative Blog Preloading & Hover Prefetcher Engine ===
window.KaghanBlogPrefetcher = {
    prefetchedUrls: new Set(),

    init: function() {
        // 1. Idle time prefetch of blogs data
        if ('requestIdleCallback' in window) {
            window.requestIdleCallback(() => KaghanBlogPrefetcher.warmCache());
        } else {
            setTimeout(() => KaghanBlogPrefetcher.warmCache(), 1000);
        }

        // 2. Speculative hover & touch prefetch on links
        document.addEventListener('mouseover', KaghanBlogPrefetcher.handleLinkInteraction, { passive: true });
        document.addEventListener('touchstart', KaghanBlogPrefetcher.handleLinkInteraction, { passive: true });
    },

    warmCache: function() {
        try {
            if (window.KaghanDB && window.KaghanDB.getBlogs) {
                window.KaghanDB.getBlogs().catch(() => {});
            }
        } catch (e) {}
    },

    handleLinkInteraction: function(e) {
        const link = e.target.closest('a');
        if (!link) return;
        const href = link.getAttribute('href');
        if (!href || href.startsWith('#') || href.startsWith('javascript:')) return;

        if (href.includes('/blog') || href.includes('blog.html') || href.includes('blog-details.html')) {
            KaghanBlogPrefetcher.prefetchPage(href);
        }
    },

    prefetchPage: function(url) {
        if (!url || KaghanBlogPrefetcher.prefetchedUrls.has(url)) return;
        KaghanBlogPrefetcher.prefetchedUrls.add(url);
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => window.KaghanBlogPrefetcher.init());
} else {
    window.KaghanBlogPrefetcher.init();
}
    ...
    buildGraph: function() {
        ...
        if (enabled.breadcrumbs) {
            ...
            if (page === 'rooms') {
                ...
                items.push({ "@type": "ListItem", "position": 2, "name": "Resort Blog", "item": `${siteUrl}/blog` });
