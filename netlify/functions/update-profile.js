...
try {
    ...
    if (e.code !== 'app/duplicate-app') {
        console.error("Firebase Admin SDK initialization failed in update-profile:", e);
    }
}
...
try {
    ...
    console.error("Firebase services retrieval failed in update-profile:", e);
}
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const { id, updatedData, idToken } = validation.data;
        ...
        // Security check: A user can only update their own profile document
        if (decodedToken.uid !== id) {
            return {
                statusCode: 403,
                headers: { 'Access-Control-Allow-Origin': allowedOrigin },
                body: JSON.stringify({ error: 'Forbidden: You cannot modify other users profiles.' })
            };
        }
        ...
        await fdb.collection('users').doc(id).update(cleanUpdate);

        const snap = await fdb.collection('users').doc(id).get();
        const updatedUser = snap.data();

        console.log(`[User Profile Update] Profile updated in Firestore for UID: ${id}`);
